...
它在 nanobot 轻量级 AI Agent 框架的基础上，深度集成了 **LangGraph 多 Agent 工作流**、**受 HyDE 启发的假设问题索引**、**Chroma + SQLite FTS5/BM25 混合知识库**，实现了从论文搜索、PDF 解析、语义入库到引用级问答的端到端闭环。
...
| **📚 智能知识库 (PaperKB)** | Chroma 向量库 + 持久化 SQLite FTS5/BM25，支持**摘要/假设问题/原文块**多视角索引 |
...
> 论文知识库支持中英文混合 BM25/词法检索和批量 embedding。未配置语义
> embedding 时会明确标记为 `hash_lexical` 降级模式；生产环境建议设置
> `embeddingFallback: "error"`，避免误把词法哈希当作语义向量。
...
| **Retrieval** | 知识库检索 | 统一查询改写 + 多视角混合检索 + 质量评估 |
...
检索：用户 Query → 多候选改写 → 并搜 Chroma 多视角 + SQLite FTS5 多字段
       → 检索族归一的加权 RRF → MMR 多样化 → 相关性过滤
```
...
```
similarity_score = 0.75×dense_similarity + 0.25×lexical_overlap
rerank_score = 0.6×similarity_score + 0.25×recency + 0.15×source_prior
```
...
# 在 ~/.nanobot/config.json 的 tools.paper 中启用自动编排；
# MinerU 可选，令牌建议通过 ${MINERU_API_TOKEN} 引用环境变量。
...
当 `tools.paper.multiAgentOrchestratorEnabled` 为 `true` 时，论文相关 Query 会自动触发多 Agent 检索，例如：
- `"最新的 Mamba 架构论文有哪些？"`
